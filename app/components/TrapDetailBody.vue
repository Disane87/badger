...
const emit = defineEmits<{
  /** Fired when the trap this view was looking at has been deleted upstream. */
  gone: []
}>()

const { data, refresh, pending } = await useFetch<{ trap: Trap; hits: Hit[] }>(
  () => `/api/traps/${props.id}`,
  { watch: [() => props.id] }
)
...
async function refreshOg() {
  ...
  try {
    await $fetch(`/api/traps/${props.id}/refresh-og`, { method: 'POST' })
    await refresh()
...
useLiveEvents((e) => {
  if (!data.value) return
  if (e.type === 'hit' && e.data.trapId === props.id) {
    if (!data.value.hits.some((h) => h.id === e.data.id)) {
      data.value.hits = [e.data, ...data.value.hits]
    }
  } else if (e.type === 'trap:updated' && e.data.id === props.id) {
    data.value.trap = e.data
  } else if (e.type === 'trap:deleted' && e.data.id === props.id) {
    emit('gone')
  }
})
</script>
